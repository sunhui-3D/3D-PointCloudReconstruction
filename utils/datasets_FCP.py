...
class GetShapenetDataset(data.Dataset):
    def __init__(self, data_dir_imgs, data_dir_pcl, models, cats, numpoints=1024,transform=False, variety=False):
    ...
    def __getitem__(self, index):
        for _ in range(1):
            ...
            pcl_path_128 = self.data_dir_pcl + self.modelnames[index] + '/pointcloud_' + str(128) + '.npy'
            pcl_path_256 = self.data_dir_pcl + self.modelnames[index] + '/pointcloud_' + str(256) + '.npy'

            if os.path.exists(pcl_path) and not os.path.exists(pcl_path_128) and not os.path.exists(pcl_path_256):
                pcl_gt_1024 = np.load(pcl_path)
                pcl_gt_128 = np.load(pcl_path_128)
                pcl_gt_256 = np.load(pcl_path_256)
            else:
                continue
            ...
            if self.variety == True:
                ...
                dis = metadata[(int(index % NUM_VIEWS))][3]
                return image, pcl_gt_128, pcl_gt_256, pcl_gt_1024, xangle, yangle

            return image, torch.Tensor(pcl_gt_128), torch.Tensor(pcl_gt_256), torch.Tensor(pcl_gt_1024)
