...
def get_loss_proj(pred, gt, device, loss_type='bce', w=1., min_dist_loss=None, dist_mat=None, opt=None):
    ...
    if min_dist_loss is not None:
        ...
        gt_white=torch.unsqueeze(torch.unsqueeze(gt, 3),3).repeat([1, 1, 1, opt.grid_h, opt.grid_w])
        #gt_white = gt_white.repeat([1,1,1,args.grid_h,args.grid_w])
        #gt_white = gt_white.repeat([1, 1, 1,64,64]).to(device)

        pred_white = torch.unsqueeze(torch.unsqueeze(pred, 3), 3).repeat([1, 1, 1, opt.grid_h, opt.grid_w])
        #pred_white = pred_white.repeat([1, 1, 1, args.grid_h, args.grid_w])
        #pred_white = pred_white.repeat([1, 1, 1, 64,64]).to(device), grid_h=opt.grid_h, grid_w=opt.grid_w
    ...
    return torch.mean(loss).cuda(), min_dist, min_dist_inv
